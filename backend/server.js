...
cloudinary.config({
  cloud_name: 'dnuebwq2e',
  api_key: '191732683294142',
  api_secret: 'xMk08Bhft40cuHZWbseL8yo6N6o',
});
...
const mongoURI = 'mongodb+srv://samriddh_kumar:sam123@tracknclassify.kjmrfft.mongodb.net/Employee?retryWrites=true&w=majority';
...
app.post('', async (req, res) => {
  try {
    ...
    const verificationLink = `http://localhost:5000/api/verify-intern/${verificationToken}`;
...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    user: 'ongcinterns@gmail.com',
    pass: 'rmir jwkq omio esra',
  },
});
...
app.post('/api/foreign-visitors', async (req, res) => {
  try {
    ...
    const verificationLink = `http://localhost:5000/api/verify-foreign/${verificationToken}`;
